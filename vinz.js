...
simbol = "⚬"
petik = '```'
fake = 'Copyright\ ©FIKRI BOT'//GANTI NAMA KAMU BEP
ban =[]
tp = "```"
// Database
    ...
    //   const gcount = setting.gcount
       ...
       // const gcount = setting.gcount
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = vinzbot.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "Copyright By ©Fikri Botz","listType": "SINGLE_SELECT","sections": list}}, {})
...
xexe = `Antilink Detector, maaf @${sender.split('@')[0]} Anda Akan Saya Tendang Ke Planet Pluto😶`		        
vinzbot.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})		    
button = [
  {buttonId: `${prefix}yo`, buttonText: {displayText: 'MAMPUS KAU'}, type: 1},
]
 buttons = {
    contentText: `${xexe}`,
    footerText: `${fake}`,
...
if (messagesC.includes('#izinadmin')) return reply('#izinadmin diterima')
var kic = `${sender.split('@')[0]}@s.whatsapp.net`
xexe = `Antilink Detector, maaf @${sender.split('@')[0]} anda akan saya tendang dari grup`		        
vinzbot.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})		    
button = [
  {buttonId: `${prefix}yo`, buttonText: {displayText: 'MAMPUS KAU'}, type: 1},
]
...
if (messagesC.includes('#izinadmin')) return reply('#izinadmin diterima')
var kic = `${sender.split('@')[0]}@s.whatsapp.net`
xexe = `Antilink Detector, maaf @${sender.split('@')[0]} anda akan di ditendang dari group`		        
vinzbot.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})		    
button = [
  {buttonId: `${prefix}yo`, buttonText: {displayText: 'MAMPUS KAU'}, type: 1},
]
...
vinzbot.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 case 'donasi':
        ...
        case 'creator':
               ...
               buttons = [{buttonId: '!youtube',buttonText:{displayText: 'YOUTUBE'},type:1},{buttonId:'ig',buttonText:{displayText:'INSTAGRAM'},type:1}]
...
`
  ...
  {buttonId: `${prefix}fikribot`, buttonText:{displayText: '/ORANG TAMVAN 🗿'}, type: 1},
  {buttonId: `${prefix}command`, buttonText: {displayText: '/COMMAND🤖'}, type: 1},{buttonId: `${prefix}sc`, buttonText: {displayText: '/SC BOT✨'}, type: 1}], "headerType": "DOCUMENT", "documentMessage": {
...
case 'nsfwloli':
                    ...
                    buttonsMessage = {footerText:'Created By ©FIKRIBOT`', imageMessage: imageMsg,
...
case 'nsfwhentai':
		            ...
		            buttons = [{buttonId: `${prefix+command}`,buttonText:{displayText: '➡️ 𝑵𝒆𝒙𝒕'},type:1},{buttonId: '❌ 𝑺𝒕𝒐𝒑',buttonText:{displayText: '❌ 𝑺𝒕𝒐𝒑'},type:1}]
                    imageMsg = ( await vinzbot.prepareMessage(from, anu, 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
                    buttonsMessage = {footerText:'Created By ©Fikri`', imageMessage: imageMsg,
                    contentText:`Hai ${pushname} Silahkan Pilih`,buttons,headerType:4}
                    prep = await vinzbot.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek, contextInfo:{"externalAdReply": {"title": `Nih Kak ${pushname}`, mediaType: 2, "thumbnailUrl": "https://telegra.ph/file/6b0259fd741e108910fbe.jpg","previewType": "VIDEO","mediaUrl": `https://youtu.be/HEq3zZ_0WCU`}}})
                    vinzbot.relayWAMessage(prep)       
                    limitAdd(sender, limit)             
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'youtube': 
              ...
              break   
case 'ig': 
              reply(`Jangan Lupa Follow Ig Owner:\n https://www.instagram.com/ropin_ganz`)
              break
case 'fikribot': 
              reply(` Owner Saya Paling Ganteng🗿`)
              break
       case 'masukandata':
...
{buttonId: `y`, buttonText: {displayText: '️❌ 𝑺𝒕𝒐𝒑'}, type: 1},
 ...
 buttons = {
contentText: `${yah}`,
footerText: `Created By ©FIKRIBOT`,
buttons: button,
headerType: 1
}
await vinzbot.sendMessage(from, buttons, MessageType.buttonsMessage, {quoted: mek})
...
{buttonId: `y`, buttonText: {displayText: '️❌ 𝑺𝒕𝒐𝒑'}, type: 1},
 ...
 buttons = {
contentText: `${gege}`,
footerText: `Created By ©FIKRI BOT`,
buttons: button,
headerType: 1
}
await vinzbot.sendMessage(from, buttons, MessageType.buttonsMessage, {quoted: mek})
...
//------------------< enable/disable>-------------------
       ...
       case 'antilink':
              ...
              antilink.push(from)
              fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
              reply('*「 ANTILINK DI AKTIFKAN 」*\n\nYang Ngirim Link Group Bakal Saya Tendang Ke Angkasa😠')
              } else if (args[0].toLowerCase() === 'disable'){
              let anu = antilink.indexOf(from)
              antilink.splice(anu, 1)
              fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
